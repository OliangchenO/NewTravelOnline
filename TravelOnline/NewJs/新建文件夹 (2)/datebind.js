...
var initTips = function () {
    $("#plandate .hasEvent").bind("click", function () {
        ...
        $(".dynamic").html($(this).attr("date") + "&nbsp;&nbsp;&nbsp;" + GetWeek($(this).attr("date")) + "&nbsp;&nbsp;&nbsp;¥" + $(this).attr("price") + "元起");
        $(".dynamic").attr("tag", $(this).attr("tag"));
        $(".dynamic").attr("date", $(this).attr("date"));
    ...
    for (var n = 0; n < newtemp.length; n++) {
        ...
        if (defaultDate > today) {
            if (newtemp[n].content != "已满") {
                ...
                if (its == 0) {
                    $(".dynamic").html(newtemp[n].date + "&nbsp;&nbsp;&nbsp;" + GetWeek(newtemp[n].date) + "&nbsp;&nbsp;&nbsp;¥" + newtemp[n].price + "元起");
                    $(".dynamic").attr("tag", newtemp[n].planid);
                    $(".dynamic").attr("date", newtemp[n].date);
        ...
        var inputData = function (element) {
            ...
            switch (s.type) {
                ...
                case "json":
                    ...
                    break;
                case "iframe":
                    $("<iframe src='" + s.source + "' marginwidth='0' marginheight='0' frameborder='0' scrolling='no' style='width:420px;height:460px;border:0;'></iframe>").appendTo($(".thickcon"));
                    $(".thickcon").removeClass(s._loading);
                    if (callback) {
                        callback()
                    };
