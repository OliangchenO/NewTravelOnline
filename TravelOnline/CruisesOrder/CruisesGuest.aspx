<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CruisesGuest.aspx.cs" Inherits="TravelOnline.CruisesOrder.CruisesGuest" %>
            ...
            <div class="long">
                <DIV id=explain class="datagrid2">
                    <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" 
                        ...
                        OnDataBound="GridView_DataBound" GridLines="Horizontal" SortExpression="IsLeader" 
                        SortDirection="desc" PageSize="50" ShowFooter="True">
	                    ...
	                    <Columns>
                            ...
                            <asp:BoundField DataField="PlanNo" HeaderText="团号" SortExpression="PlanNo">
		                        <HeaderStyle Width="6%" />
                            </asp:BoundField>
                            <asp:BoundField DataField="RoomNo" HeaderText="房号" SortExpression="RoomNo">
		                        <HeaderStyle Width="6%" />
                            </asp:BoundField>
                            <asp:BoundField DataField="TabelNo" HeaderText="餐桌" SortExpression="TabelNo">
		                        <HeaderStyle Width="6%" />
                            </asp:BoundField>
                            <asp:BoundField DataField="DinnerTime" HeaderText="用餐时间" SortExpression="DinnerTime">
		                        <HeaderStyle Width="6%" />
                            </asp:BoundField>
                            ...
                            <asp:TemplateField HeaderText="其他">
                                <ItemTemplate>
                                    <input value="<%# DataBinder.Eval(Container.DataItem, "otherinfo") %>" class="OtherInput" type="text" name="OtherInput" id="<%# DataBinder.Eval(Container.DataItem, "Id") %>" maxlength="10" style="width: 60px"/>
                                </ItemTemplate>
                                <HeaderStyle Width="5%" />
                            </asp:TemplateField>
	                    </Columns>
    ...
    <script type="text/javascript">
        ...
        $(function () {
            $('.OtherInput').blur(function () {
                if (inputs != $(this).val())  SaveOtherInfo($(this).attr("id"), $(this).val());
            });
        ...
        function SaveOtherInfo(cid, infos) {
            var url = "AjaxService.aspx?action=SaveGuestOtherInfo&cid=" + cid + "&otherinfo=" + infos + "&r=" + Math.random();
            $.get(url, function (data) {
                var obj = eval(data);
                if (obj.success) {
                    jSuccess('<strong>' + infos + ' 保存成功!</strong>', { ShowOverlay: false, HorizontalPosition: 'center', VerticalPosition: 'center' });
                }
                if (obj.error) {
