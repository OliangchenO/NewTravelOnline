<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CruisesSetDinner.aspx.cs" Inherits="TravelOnline.CruisesOrder.CruisesSetDinner" %>
...
<form id="form1" runat="server">
        <div class="main">
            <div id="order_title">
            <h2 class="headline"><SPAN class=headstep><%=CruisesShip%> - 餐桌号安排</SPAN></h2>
            </div>
            <DIV id="DIV1" style="DISPLAY:none">
                ...
                <input id="DinnerId" name="DinnerId" type="hidden" value=""/>
            </DIV>
            <div class="serch">
                要求时段: <asp:DropDownList ID="DropDownList1" runat="server" DataTextField="DinnerClaim" DataValueField="DinnerClaim" Width="60px"></asp:DropDownList>
                ...
                <DIV id=explain class="datagrid2">
                    <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" 
	                    ...
	                    <Columns>
                            ...
                            <asp:BoundField DataField="DinnerClaim" HeaderText="要求时段" SortExpression="DinnerClaim">
		                        <HeaderStyle Width="10%" />
                            </asp:BoundField>
                            <asp:TemplateField HeaderText="用餐时间" SortExpression="DinnerTime">
                                <ItemTemplate>
                                    <span id="Span_<%# DataBinder.Eval(Container.DataItem, "Id") %>"><%# DataBinder.Eval(Container.DataItem, "DinnerTime")%></span>
                                </ItemTemplate>
                                <HeaderStyle Width="10%" />
                            </asp:TemplateField>
                            <asp:TemplateField HeaderText="桌号" SortExpression="TabelNo">
                                <ItemTemplate>
                                    <input value="<%# DataBinder.Eval(Container.DataItem, "TabelNo") %>" class="sel RoomNoInput" type="text" name="RoomInput" id="<%# DataBinder.Eval(Container.DataItem, "Id") %>" tag="<%# DataBinder.Eval(Container.DataItem, "OrderNums") %>" Claim="<%# DataBinder.Eval(Container.DataItem, "DinnerClaim") %>" maxlength="10" style="width: 160px"  readonly="readonly"/>
                                </ItemTemplate>
...
<uc3:ManagerFooter ID="ManagerFooter1" runat="server" />
    <script type="text/javascript">
        $('.RoomNoInput').bind('click', function () {
            ...
            var url = "../Common/GetAutoDropList.aspx?action=ShowCruisesDinnerNo&SerchName=" + $("#LineId").val() + "&nums=" + $(this).attr("tag") + "&Claim=" + $(this).attr("Claim");
            show(this, "DinnerId", url, "yes");
        });

        function afterselet(callback) {
            var TabelNo = $("#" + $("#GuestId").val()).val();
            if (TabelNo == "") return;
            var url = "AjaxService.aspx?action=SetCruisesDinnerNo&GuestId=" + $("#GuestId").val() + "&DinnerId=" + $("#DinnerId").val() + "&r=" + Math.random();
            $.getJSON(url, function (date) {
                if (date.success == 0) {
                    $("#Span_" + $("#GuestId").val()).html(date.DinnerTime);
        ...
        function GroupDo(flag) {
            ...
            var arrChk = "";
            var doname = "自动分配餐桌号"
            var action = "CruisesAutoDinnerNo"
            if (flag == "Cancel") {
                doname = "取消已分配餐桌号";
                action = "CruisesDinnerNoCancel"
            }
            ...
            if (flag == "Group") {
                ...
                if (arrChk.split(',').length > 15) {
                    jNotify('<strong>选择的人数过多，请重新选择!</strong>', { ShowOverlay: false, HorizontalPosition: 'center', VerticalPosition: 'center' });
                    return false;
                }
                var url = "SelectDinnerPlan.aspx?flag=Dinner&LineId=" + $("#LineId").val() + "&Id=" + arrChk + "&r=" + Math.random();
                var dg = new $.dialog({ id: 'No1', page: url, title: '餐桌分配', width: 650, height: 470, skin: 'aero', btnBar: false, iconTitle: false, cover: true });
                dg.ShowDialog();
            }
            if (flag == "Cancel") {
