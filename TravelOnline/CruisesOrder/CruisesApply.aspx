<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="CruisesApply.aspx.cs" Inherits="TravelOnline.CruisesOrder.CruisesApply" %>
...
href="/Management/ManageHome.aspx">管理中心首页</A>&nbsp;&gt;&nbsp;<SPAN>订单变更申请</SPAN></DIV>


 <DIV class="m select">
    <form id="formlogin" runat="server">
    <DIV class=mt>
        <H1></H1><STRONG>邮轮包船订单变更申请</STRONG>
    </DIV>
    <div class="toolbar">&nbsp;
        <asp:DropDownList ID="DropDownList1" runat="server" Width="120">
            <asp:ListItem Value="1">最近一月申请</asp:ListItem>
            <asp:ListItem Value="2">历史申请</asp:ListItem>
        </asp:DropDownList>
        &nbsp;
        <asp:DropDownList ID="DropDownList2" runat="server" Width="120">
            ...
            <asp:ListItem Value="DinnerChange">用餐变更</asp:ListItem>
            <asp:ListItem Value="Settlement">结算方式</asp:ListItem>
        </asp:DropDownList>
        &nbsp;
        ...
        <DIV id="inputs" style="DISPLAY:none">
            ...
            <input id="DoFlag" type="hidden" value="0"/>
    ...
    <DIV id=explain class="datagrid">
        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" 
	        ...
	        <Columns>
                ...
                <asp:BoundField DataField="infos" HeaderText="调整内容" SortExpression="infos">
		            <HeaderStyle Width="40%" />
                    <ItemStyle HorizontalAlign="Left" />
                </asp:BoundField>
                <%--<asp:BoundField DataField="roomname" HeaderText="原舱房" SortExpression="roomname">
		            <HeaderStyle Width="20%" />
                    <ItemStyle HorizontalAlign="Left" />
                </asp:BoundField>
                <asp:BoundField DataField="newroomname" HeaderText="新舱房" SortExpression="newroomname">
		            <HeaderStyle Width="20%" />
                    <ItemStyle HorizontalAlign="Left" />
                </asp:BoundField>--%>
                ...
                <asp:BoundField DataField="inputdate" HeaderText="申请日期" SortExpression="inputdate" DataFormatString="{0:yy-MM-dd}">
		            <HeaderStyle Width="8%" />
                </asp:BoundField>
    ...
    <div id="toolbars" class="tips">
	    <div class="tips-text">
		   <a id=A1 href="javascript:void(0);" class="easyui-linkbutton" plain="true" iconCls="icon-edit">申请确认</a>
           <a id=A2 href="javascript:void(0);" class="easyui-linkbutton" plain="true" iconCls="icon-edit">申请取消</a>
	    </div>
...
<DIV class=clr></DIV></DIV>
    ...
    <script type="text/javascript">
        ...
        function DoAdjust(applyid) {
            if (confirm('确认操作您选择的申请吗？')) {
            }
            else
            { return false; }
            var url = "AjaxService.aspx?action=OrderApplyDoAdjust&ApplyId=" + applyid + "&doflag=" + $("#DoFlag").val() + "&r=" + Math.random();
            //window.open(url);
        ...
        function DoCancel(applyid) {
            ...
            { return false; }
            var url = "AjaxService.aspx?action=CruisesRoomDoCancel&ApplyId=" + applyid + "&r=" + Math.random();
            $.get(url, function (data) {
                var obj = eval(data);
                if (obj.success) {
                    alert("变更申请取消成功!");
