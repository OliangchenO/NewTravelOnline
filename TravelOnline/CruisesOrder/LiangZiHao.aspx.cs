...
{
    public partial class LiangZiHao : System.Web.UI.Page
    {
                ...
                for (int i = 0; i < DS.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (drs.Count() > 0)
                    {
                        ...
                        foreach (DataRow dr in drs)
                        {
                            ...
                            ICell cel4 = row1.CreateCell(4 + ii); //第一个客人姓的拼音
                            ICell cel5 = row1.CreateCell(5 + ii); //第一个客人名的拼音
                            ...
                            if (GuestName.Length < 5)
                            {
                                ...
                                ICell cel8 = row1.CreateCell(8 + ii); //第一个客人姓的拼音
                                ICell cel9 = row1.CreateCell(9 + ii); //第一个客人名的拼音
                            ...
                            ICell cel13 = row1.CreateCell(13 + ii); //第一个客人的用餐批次
                            cel13.SetCellValue(dr["DinnerTime"].ToString());

                            ICell cel14 = row1.CreateCell(14 + ii); //第一个客人的船票类型(只放C/O)
                            cel14.SetCellValue("C/O");

                            ICell cel15 = row1.CreateCell(15 + ii); //第一个客人的国籍
                            ...
                            ICell cel20 = row1.CreateCell(20 + ii); //第一个客人的护照号
                            ICell cel21 = row1.CreateCell(21 + ii); //第一个客人的护照有效期
        ...
        protected void ManifestUploadExcel()
        {
            ...
            if (Action == "AllCharterManifest")
            {
                SqlQueryText = string.Format("SELECT * from CR_RoomNo where berth<5 and Lineid={0} order by id", Request.QueryString["lineid"]);
            }
            else
            {
                SqlQueryText = string.Format("SELECT * from CR_RoomNo where berth<5 and id in (select RoomNoid from View_GuestRoomInfo where lineid='{0}' and PlanAllotid in ({1}) group by RoomNoid) order by id", Request.QueryString["lineid"], Cid);
            }
            DataSet DS = new DataSet();
            DS.Clear();
            DS = MyDataBaseComm.getDataSet(SqlQueryText);
                    ...
                    if (drs.Count() > 0)
                    {
                        ...
                        foreach (DataRow dr in drs)
                        {
                            ...
                            if (dr["IdNumber"].ToString().Contains("/"))
                            {
                                ...
                                cel20.SetCellValue(idNumbers[0]);
                            }
                            else
                            {
                                cel20.SetCellValue(dr["IdNumber"].ToString());
