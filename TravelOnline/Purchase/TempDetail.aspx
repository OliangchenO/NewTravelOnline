<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TempDetail.aspx.cs" Inherits="TravelOnline.Purchase.TempDetail" %>
    ...
    <div class="m detail">
        ...
        <div class="mc tabcon borders01">
            ...
            <ul id=orderinfo class=order>
                <li><div class=oname1><span class=xh>*</span>联系人：</div><div class=oinfo_n><input class=oinfodetail id="Order_Name" name="Order_Name" type="text" maxlength="30" style="width: 100px" value="<%=User_Name %>" dataType="Require" msg="联系人不能为空"/>&nbsp;<input type=checkbox name="SaveInfos" id="SaveInfos" checked=checked />保存联系人信息</div></li>
                <li><div class=oname1><span class=xh>*</span>手机号码：</div><div class=oinfo_n><input class=oinfodetail id="Order_Mobile" name="Order_Mobile" type="text"  maxlength="20" style="width: 200px" value="<%=User_Mobile %>" dataType="Mobile" msg="手机号码格式不正确" require="false"/>&nbsp;<span class=hs>手机号码和联系电话至少填写一项</span></div></li>
                <li><div class=oname1><span class=xh></span>联系电话：</div><div class=oinfo_n><input class=oinfodetail id="Order_Tel" name="Order_Tel" type="text" maxlength="20" style="width: 200px" value="<%=User_Tel %>" dataType="Phone" msg="联系电话格式不正确" require="false"/>&nbsp;<span class=hs>示例：021-64330000</span></div></li>
                <li><div class=oname1>传真号码：</div><div class=oinfo_n><input class=oinfodetail id="Order_Fax" name="Order_Fax" type="text"  maxlength="20" style="width: 200px" value="<%=User_Fax %>" dataType="Phone" msg="传真号码格式不正确" require="false"/>&nbsp;</div></li>
                <li><div class=oname1><span class=xh>*</span><%=dianziyoujian %>：</div><div class=oinfo_n><input class=oinfodetail id="Order_Email" name="Order_Email" type="text"  maxlength="50" style="width: 200px" value="<%=User_Email %>" dataType="Email" msg="电子邮件不能为空或格式不正确"/>&nbsp;</div></li>
                <li class=memo><div class=oname2><%=tebieshuoming %>：</div><div class="oinfo_n memo"><textarea name="Order_Memo" rows="2" cols="20" id="Order_Memo" name="Order_Memo" style="width: 500px; height: 50px;" onkeydown="limitChars(this, 100)" onchange="limitChars(this, 100)" onpropertychange="limitChars(this, 100)"><%=User_Memo %></textarea>&nbsp;<span class=hs>100字以内</span></div></li>
            </ul>
        </div>
    </div>
...
<%--            <div class=guest>
    ...
    <div class="m detail <%=mendian_hide%>">
...
<form id="form_data" onsubmit="return false;" method="post">
    ...
    <input id="CruisesFlag" name="CruisesFlag" type="hidden" value="0"/>
    <input id="huifeng" name="huifeng" type="hidden" value="<%=huifengid %>"/>
</form>
<DIV class=clr></DIV></DIV>
    ...
    <script type="text/javascript">
        ...
        function SubmitOrder() {
            if ($("#huifeng").val() == "1")
            {
                if ($("#Order_Memo").val().length < 3) {
                    alert("会员工号不能为空，出行游客中有几位会员须填写几位工号");
                    return false;
                }
            }
            var OrderInfo = "";
...
//            return false;
            ...
            $.post(url, $("#form_data").serialize(), function (data) {
                var obj = eval(data);
                if (obj.success) {
                    top.location = "/Order/ThirdStep/" + $('#TempOrderId').val() + ".html";
                }
                else {
                    $("#islogin").hide();
                    $("#nextstep").show();
...
//            var url = "/Purchase/AjaxService.aspx?action=SecondStep&SaveFlag=" + SaveFlag + "&NFP=" + NeedInvoice + "&TempOrderId=" + $('#TempOrderId').val() + "&OrderInfo=" + escape(OrderInfo) + "&GuestInfo=" + escape(Parms) + "&HT=" + escape(HT) + "&FP=" + escape(FP) + "&r=" + Math.random();
