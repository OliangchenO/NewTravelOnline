<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="OrderNow.aspx.cs" Inherits="TravelOnline.Purchase.OrderNow" %>
...
<body>
    ...
    <UL class=tab><LI class=curr>选择人数<SPAN></SPAN></LI></UL>
        <div id="step1" <%=step1 %> class="mc tabcon borders">
            <div id="DateList" class="mc tabcon"><STRONG><%=OrderInfo %></STRONG></div>
            <div style="padding-top: 15px;LINE-HEIGHT: 20px;">
                <span style="padding-left: 20px">成人</span> <input id="AdultNums" type="text" class="easyui-numberbox" precision="0" max="9" size="3" maxlength="1" style="text-align:center;" value="<%=nums %>"/> 人 (最少成行：1人)
                <span style="padding-left: 20px">儿童</span> <input id="ChildNums" type="text" class="easyui-numberbox" precision="0" max="9" size="3" maxlength="1" style="text-align:center;" value="<%=etnums %>"/> 人 (2-12岁)
                
            </div>
        </div><!--tabcon end-->
        <div id="step2" <%=step2 %> class="mc tabcon borders">
            <div id="DateList1" class="mc tabcon">
                <SPAN style="LINE-HEIGHT: 55px; PADDING-LEFT: 50px; color: #009900; font-size: 16px; font-weight: bold;"><%=begindate %> 出发的报名人数已满，请重新选择！</SPAN>
            </div>
        </div>
...
希望全体旅游嘉宾积极配合领队和导游的工作，互相关爱，互相帮助，团结协作，共同完成这次愉快的旅行。如果您结束旅游时，脸上依然还带着阳光般的笑容，那就是您对我们最高的奖赏。<br />
    ...
    <script type="text/javascript">
        ...
        function BuyNow() {
            ...
            if (nums > Number($('#PlanNums').val()) || nums > 9 ) {
                alert("报名人数超过了可报名的剩余数量！");
                return false;
            };
            $("#islogin").show();
            $("#OrderBtn").hide();
