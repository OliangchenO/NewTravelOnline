...
drawLinks();});};
/*getSortData*/
var category={getDataUrl:"http://www.360buy.com/ajaxservice.aspx?stype=SortJson",getBrandsUrl:"http://www.360buy.com/lishi.aspx?callback=category.getBrandService&id=a,915,925^b,916,926^c,917,927^d,918,928^e,919,929^f,920,930^g,921,931^h,922,932^i,923,933^j,924,934^k,2096,2097",template:["<div class='{0}'>{1}</div>","<span><h3>{0}</h3><s></s></span>","<div class='i-mc'><div onclick=\"$(this).parent().parent().removeClass('hover')\" class=\"close\"></div><div class='subitem'>{0}</div><div id='JD_sort_{1}' class='fr'><div class='iloading'>正在加载中，请稍候...</div></div></div>","<dl class='{0}'><dt>{1}</dt><dd>{2}</dd></dl>","<em>{0}</em>","<a href='{0}'>{1}</a>","<div class='extra'><a href='http://www.360buy.com/allSort.aspx'>全部商品分类</a></div>","<ul>{0}</ul>","<li>·{0}</li>"],getString:function(a,d){if(!typeof d=="object"&&!d.length){return;}var e,c;for(var b=0;b<d.length;b++){c=new RegExp("\\{"+b+"\\}");e=e?e.replace(c,d[b]):a.replace(c,d[b]);}return e;},getUrl:function(a){if(/^http[s]?:\/\/([\w-]+\.)+[\w-]+([\w-.\/?%&=]*)?$/.test(a)){return a;}else{return"http://www.360buy.com/products/"+a+".html";}},getData:function(){$.getJSONP(this.getDataUrl,category.getDataService);},getDataService:function(t){$("#_JD_ALLSORT").attr("load","1");var C=t.data,A=this.getString,o=this.getUrl,B=this.template,h=[];for(var w=0;w<C.length;w++){var g=(w==0)?"item fore":"item",f=(!C[w].u)?C[w].n:A(B[5],[C[w].u,C[w].n]),e=[],d=[],s=C[w].i,n,m;for(var v=0;v<s.length;v++){n=s[v].i;var l=[],k=(s[v].u)?A(B[5],[o(s[v].u),s[v].n]):s[v].n;for(var u=0;u<n.length;u++){m=n[u].split("|");var j=A(B[5],[o(m[0]),m[1]]);l.push(A(B[4],[j]));}d.push(A(B[3],["",k,l.join("")]));}e.push(A(B[1],[f]));e.push(A(B[2],[d.join(""),C[w].t]));h.push(A(B[0],[g,e.join("")]));}h.push(B[6]);$("#_JD_ALLSORT").html(h.join(""));$(".allsort .item").hoverForIE6({delay:150});this.getBrands();},getBrands:function(){$.getJSONP(this.getBrandsUrl,category.getBrandService);},getBrandService:function(t){$("#_JD_ALLSORT").attr("load","2");var h=t.data,f=this.getString,m=this.getUrl,r=this.template;for(var q=0;q<h.length;q++){var e=h[q]["b"],d=h[q]["p"],g=h[q]["id"],i=[],p=[],o=[],l;if(e.length>0){for(var n=0;n<e.length;n++){l=f(r[5],[e[n]["u"],e[n]["n"]]);p.push(f(r[4],[l]));}var j=(g=="k")?"brands brands-book":"brands",s=(g=="k")?"推荐品牌出版商":"推荐品牌";i.push(f(r[3],[j,s,p.join("")]));}else{i.push(f(r[0],["norecode","暂无推荐！"]));}if(d.length>0){for(var k=0;k<d.length;k++){l=f(r[5],[d[k]["u"],d[k]["n"]]);o.push(f(r[8],[l]));}o=f(r[7],[o.join("")]);i.push(f(r[3],["promotion","促销活动",o]));}else{i.push(f(r[0],["norecode","暂无促销活动！"]));}$("#JD_sort_"+g).html(i.join(""));}},init:function(){var a=this;if ($("#_JD_ALLSORT").length==0&&$("#o-search").length!=0){a.getBrands();return;}$("#_JD_ALLSORT").one("mouseover",function(){if(!$("#_JD_ALLSORT").attr("load")){a.getData();}else{if($("#_JD_ALLSORT").attr("load")==1){a.getBrands();}else{return;}}});}};category.init();
/*jdThickbox*/
