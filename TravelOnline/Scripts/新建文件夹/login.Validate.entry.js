...
$.extend(validateFunction,{
	...
	FORM_validate:function(){
		$("#loginname").jdValidate(validatePrompt.loginmail,validateFunction.loginmail,true);
...
$("#loginname").jdValidate(validatePrompt.loginmail,validateFunction.loginmail);
...
$("#loginsubmit").click(function() {
    ...
    if (flag) {
        ...
        $.ajax({
            type: "POST",
            url: "/Login/LoginService.aspx?uuid=" + uuid + "&r=" + Math.random(),
            ...
            success: function(result) {
            	if (result) {
                	//var obj = eval("("+result+")");
                	var obj = eval(result);
                    var urls=DeEight(location.search.substring(1));
                    if (urls=="") 
                    	urls="/index.html";
                    if (obj.success)
