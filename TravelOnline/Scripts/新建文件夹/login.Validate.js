...
var validatePrompt={
	username:{
		...
		isNull:"请输入用户名",
		error:{
			beUsed:"该用户名已被使用，请使用其它用户名注册，如果您是&quot;{1}&quot;，请<a href='login.aspx' class='flk13'>登录</a>",
	...
	mail:{
		onFocus:"请输入常用的邮箱，将用于登录账号、接收订单通知等",
		succeed:"",
		isNull:"请输入常用的邮箱，将用于登录账号、接收订单通知等",
		error:{
		...
		if(!length&&format){
	        ...
	        if(!namestate||nameold!=option.value)
	        {
	            if(nameold!=option.value)
	            {
		            ...
		            $.getJSON("/Login/AjaxService.aspx?action=CheckManageName&uid="+escape(option.value)+"&r="+Math.random(),function(date){
			        if(date.success==0){
	...
	authcode:function(option){
		...
		if(!authcodestate||authcodeold!=option.value)
	    {
	        if(authcodeold!=option.value)
	        {
	            ...
	            alert("AjaxService.aspx?action=CheckAuthcode&str="+escape(option.value)+"&r="+Math.random()+"&uuid="+uuid);
