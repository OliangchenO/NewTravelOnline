...
/*getSortData*/
var category = {
    getDataUrl: "/ajaxservice.aspx?stype=SortJson",
    getBrandsUrl: "/lishi.aspx?callback=category.getBrandService&id=a,915,925^b,916,926^c,917,927^d,918,928^e,919,929^f,920,930^g,921,931^h,922,932^i,923,933^j,924,934^k,2096,2097",
    template: ["<div class='{0}'>{1}</div>", "<span><h3>{0}</h3><s></s></span>", "<div class='i-mc'><div onclick=\"$(this).parent().parent().removeClass('hover')\" class=\"close\"></div><div class='subitem'>{0}</div><div id='JD_sort_{1}' class='fr'><div class='iloading'>正在加载中，请稍候...</div></div></div>", "<dl class='{0}'><dt>{1}</dt><dd>{2}</dd></dl>", "<em>{0}</em>", "<a href='{0}'>{1}</a>", "<div class='extra'><a href='/allSort.aspx'>全部商品分类</a></div>", "<ul>{0}</ul>", "<li>·{0}</li>"],
...
/*picMarquee*/
 (function($) {
    $.fn.picMarquee = function(option, callback) {
        if (typeof option == "function") {
            callback = option;
            option = {}
        };
...
var Operator = function() {
    ...
    this.selectText = function(_d) {
        ...
        $callback.hidden();
        window.location.href = "/search?keyword=" + _d.title;
    };
...
/* friend */
var jdFriendUrl = '/TuiJianService.aspx';
...
/*jdCalcul*/
 (function($) {
    $.jdCalcul = function(pids) {
        var arr = null;
        var pids = pids.join(",");
        var dataUrl = "/limitBuy.aspx?callback=?&ids=" + pids;
        var purl = "/product/";
...
var jdModelCallCenter = {
    ...
    login: function() {
        ...
        if (flag) {
            location.href = "/new/login.aspx?ReturnUrl=" + escape(location.href);
            return;
        }
        setTimeout(function() {
            $.jdThickBox({
                type: "iframe",
                title: "您尚未登录",
                source: "/new/LoginFrame.aspx?clstag1=" + _this.settings.clstag1 + "&clstag2=" + _this.settings.clstag2 + "&r=" + Math.random(),
                width: 450,
                height: 360,
                _title: "thicktitler",
                _close: "thickcloser",
    ...
    regist: function() {
        ...
        setTimeout(function() {
            $.jdThickBox({
                type: "iframe",
                title: "您尚未登录",
                source: "/new/registPersonalFrame.aspx?clstag1=" + _this.settings.clstag1 + "&clstag2=" + _this.settings.clstag2 + "&r=" + Math.random(),
                width: 450,
                height: 500,
                _title: "thicktitler",
                _close: "thickcloser",
    ...
    init: function() {
        var _this = this;
        $.getJSON("/new/loginwebservice.aspx?callback=?", {
            method: "GetHello"
        },
        function(result) {
            _this.tbClose();
...
$.login = function(options) {
    options = $.extend({
        loginService: "/loginservice.aspx?callback=?",
        loginMethod: "Login",
...
var miniCartService = {
    settings: {
        url: "/purchase/minicartservice.aspx?tmp=0&callback=?",
        amountCookie: "cn",
...
$.extend(jdModelCallCenter, {
    saveFavorite: function(productId) {
        var homeServiceUrl = "/homeservice.aspx?callback=?";
        $.login({
            modal: true,
                ...
                if (result != null && result.IsAuthenticated != null && result.IsAuthenticated) {
                    if (productId > 0) {
                        ...
                        function(result) {
                            ...
                            $.getJSON("/regard/save.action?goodsId=" + productId + "&jsoncallback=?", 
                            function(json) {})
                        })
                    }
                }
...
//AutoGetId设置id是否从传递参数或页面获取，0为不获取 
function refreshComments(referenceFlag, referenceType, parameter, AutoGetId) {
    var referenceId;
    if (AutoGetId == 0) {
        referenceId = "0";
    }
    else { 
        referenceId = $.query.get("id"); //"185688"//
        if (referenceId == "") {
    ...
    if (referenceId != "") {
        switch (referenceFlag) {
            case "IndexProductList":
                url = "/Js/ProductList/{0}.js".format(referenceType);
                $.getJSONP(url, GetProductList);
                break;
            case "1":
...
function GetProductList(result) {
    var DivName = "#" + result.ReferenceType
    $(DivName).html(result.Result);
    //$("#Text1").val(result.Result);
