...
$.extend(validateFunction,{
	...
	remark:function(option){
		var length=validateRules.betweenLength(option.value,0,100);
		if(!length){
			validateSettings.error.run(option,option.prompts.error.badLength);
		}
		else {
	...
	FORM_validate:function(){
		//var length=validateRules.betweenLength($("#zipcode").val(),0,100);
		...
		return validateFunction.FORM_submit(["#authcode","#truename","#tel","#mobile","#zipcode","#address"]) //,"#address","#zipcode","#remark"
	}		 
});
