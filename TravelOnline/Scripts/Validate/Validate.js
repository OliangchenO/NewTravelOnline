...
var validatePrompt = {
	...
	mail:{
		onFocus:"请输入常用的邮箱，将作为您的登陆用户名",
		succeed:"",
		isNull:"请输入常用的邮箱，将作为您的登陆用户名",
		error:{
			beUsed:"该邮箱已被使用，请更换其它邮箱，或使用该邮箱<a href='retrievepassword.aspx' class='flk13'>找回密码</a>",
			badFormat:"邮箱格式不正确",
			badLength:"您填写的邮箱过长，邮件地址只能在50个字符以内"
		}
    },
    findmail: {
        onFocus: "请输入您的登陆用户名（电子邮箱地址）",
        succeed: "",
        isNull: "请输入您的登陆用户名（电子邮箱地址）",
        error: {
            beUsed: "您的登陆用户名（电子邮箱地址）不存在，请重新输入",
            badFormat: "邮箱格式不正确",
            badLength: "您填写的邮箱过长，邮件地址只能在50个字符以内"
        }
    },
...
var validateFunction={
	...
	mail:function(option){
		...
		var format2=validateRules.betweenLength(option.value,5,50);
		if(!format){
			validateSettings.error.run(option,option.prompts.error.badFormat);
		}else{
			if (!format2){
		        ...
		        if(!emailstate||emailold!=option.value)
		        {
		            if(emailold!=option.value)
		            {
		                ...
		                option.errorEle.html("<span style='color:#999'>邮件地址检验中……</span>");
		                $.getJSON("/Login/AjaxService.aspx?action=CheckUemail&str="+escape(option.value)+"&r="+Math.random(),function(date){
			                if(date.success==0){
				                validateSettings.succeed.run(option);
				                emailstate=true;
			                }else{
            ...
            if (!format2) {
                ...
                if (!emailstate || emailold != option.value) {
                    if (emailold != option.value) {
                        emailold = option.value;
                        option.errorEle.html("<span style='color:#999'>邮件地址检验中……</span>");
                        $.getJSON("/Login/AjaxService.aspx?action=FindEmail&str=" + escape(option.value) + "&r=" + Math.random(), function (date) {
                            if (date.success == 0) {
                                validateSettings.succeed.run(option);
                                emailstate = true;
                            } else {
	...
	authcode:function(option){
	    if(!authcodestate||authcodeold!=option.value)
	    {
	        if(authcodeold!=option.value)
	        {
	            ...
	            option.errorEle.html("<span style='color:#999'>验证码检验中……</span>");
	            var uuid=$("#JD_Verification1").attr("src").split("&uid=")[1].split("&")[0];
		        $.getJSON("/Login/AjaxService.aspx?action=CheckAuthcode&str="+escape(option.value)+"&r="+Math.random()+"&uuid="+uuid,function(date){
			        if (date.success==0){
				        validateSettings.succeed.run(option);
				        authcodestate=true;
			        }else{
