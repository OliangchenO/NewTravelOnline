...
jQuery.fn.GetPinYin = function () {
    var parentid = "#" + $(this).parents("span").attr("id");
    var cnname = $.trim($(this).val());
    if (cnname.length < 2 || cnname == "中文姓名") {
        $(this).attr({ "oldname": "" });
...
jQuery.fn.CheckIdCard = function () {
    ...
    if (idcard.length == 15 || idcard.length == 18) {
        checkIdcard(idcard);
        $(parentid + " input:eq(2)").val(C_BirthDay);
        $(parentid + " input:eq(2)").css('color', '#000');
    }
};
...
$(document).ready(function () {
    ...
    $(".guest :input").each(function () {
        ...
        if (classname == "Guest_PassBgn" || classname == "Guest_PassEnd") $(this).smartFocus(tip_brithday);
        if (classname == "Guest_Name") $(this).smartFocus(tip_guestname);
        if (classname == "Guest_EnName") $(this).smartFocus(tip_enname);
    });
    $(".Guest_BirthDay").each(function () {
        $(this).smartFocus(tip_brithday);
    });
});
...
function CheckForm() {
    $(".guest .GuestDiv").each(function () {
        var parentid = "#" + $(this).attr("id");
        if ($(parentid + " input:eq(0)").val() == "中文姓名") {
...
//            return false;
//        }
        if ($(parentid + " input:eq(2)").val() == "yyyy-mm-dd") {
            $(parentid + " input:eq(2)").SetErrorInfo("出生日期必须填写,格式为yyyy-mm-dd，示例：1988-01-01");
            CheckFlag = "1";
            return false;
        }
        else {
            if ($(parentid + " input:eq(2)").DateErrorInfo("出生日期填写错误,格式为yyyy-mm-dd，示例：1988-01-01") == false) return false;
        }
        if ($(parentid + " input:eq(5)").val() != "yyyy-mm-dd") { 
            if ($(parentid + " input:eq(5)").DateErrorInfo("签发日期填写错误,格式为yyyy-mm-dd，示例：1988-01-01") == false) return false;
        }
        if ($(parentid + " input:eq(6)").val() != "yyyy-mm-dd") {
            if ($(parentid + " input:eq(6)").DateErrorInfo("签发日期填写错误,格式为yyyy-mm-dd，示例：1988-01-01") == false) return false;
        }
    });
