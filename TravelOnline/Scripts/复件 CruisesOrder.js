...
$(function () {
    $('.tip').live("mouseover", function () {
        $('#tip').show('fast');
        var pic = $(this).attr("tag");
        var infos = "<IMG onerror=\"this.src='/Images/none.gif'\" src=" + $(this).attr("tag") + " width=300>";
        $('#olog').html(infos);
...
function calTotal(pid) {
    ...
    switch (berth) {
        ...
        case 3:
            total = hourseNum * price1 * 2;
            if (childs >= hourseNum) total += hourseNum * price3;
            if (childs < hourseNum) {
                total += childs * price3;
                total += (hourseNum - childs) * price2;
            }
            break
        case 4:
            total = hourseNum * price1 * 2;
            if (childs >= hourseNum * 2) {
                total += hourseNum * 2 * price3;
            }
            else { 
                total += childs * price3;
                total += (hourseNum * 2 - childs) * price2;
            }
//            if (childs >= hourseNum) {
//                total += hourseNum * price2;
//                total += hourseNum * price3;
//            }
//            if (childs < hourseNum) {
//                total += childs * price3;
//                total += (hourseNum * 2 - childs) * price2;
//            }
        ...
        case 5:
            total = hourseNum * price1 * 2;
            if (childs >= hourseNum * 3) {
                total += hourseNum * 3 * price3;
            }
            else {
                total += childs * price3;
                total += (hourseNum * 3 - childs) * price2;
            }
            break
        case 6:
            total = hourseNum * price1 * 2;
            if (childs >= hourseNum * 4) {
                total += hourseNum * 4 * price3;
            }
            else {
                total += childs * price3;
                total += (hourseNum * 4 - childs) * price2;
            }
            break
        default:
            total = hourseNum * price1 * 2;
