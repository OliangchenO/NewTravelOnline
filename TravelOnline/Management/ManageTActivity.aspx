<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ManageTActivity.aspx.cs" Inherits="TravelOnline.Management.ManageTActivity" %>
...
<form id="form1" runat="server">
        <div class="main">
            <div id="order_title">
            <h2 class="headline"><SPAN class=headstep>活动管理</SPAN></h2>
            </div>
            <div class="toolbar">
            ...
            <div id=explain class="datagrid">
                <input type="hidden" id="tradingAreaId" name="tradingAreaId" value="<%=tradingAreaId %>"
                <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" CellPadding="4" ForeColor="#333333" AllowSorting="True"  OnSorting="GridView_Sorting"
	                ...
	                <Columns>
                        ...
                        <asp:BoundField DataField="title" HeaderText="活动简介" SortExpression="title">
		                    <HeaderStyle Width="10%" />
                        </asp:BoundField>
                        <asp:BoundField DataField="context" HeaderText="活动内容" SortExpression="context">
...
<uc3:ManagerFooter ID="ManagerFooter1" runat="server" />
    <script type="text/javascript">
        ...
        $(function () {
            $('#AddNew').dialog({ id: 'No1', page: 'TActivityInfoAdd.aspx?tradingAreaId=' + $('#tradingAreaId').val(), title: '增加活动信息', width: 590, height: 580, fixed: true, btnBar: false, skin: 'aero', iconTitle: false, maxBtn: false, cover: true });
        });

        function EditDes(id, tradingAreaId) {
            var url = "TActivityInfoAdd.aspx?Cid=" + id + "&tradingAreaId=" + tradingAreaId;
            var dg = new $.dialog({ id: 'No1', page: url, title: '活动信息修改', width: 600, height: 400, skin: 'aero', btnBar: false, iconTitle: false, cover: true });
            dg.ShowDialog();
        }
        ...
        function DeleteSelect() {
            ...
            var url = "AjaxService.aspx?action=DeleteTActivityInfo&Id=" + arrChk + "&r=" + Math.random();
