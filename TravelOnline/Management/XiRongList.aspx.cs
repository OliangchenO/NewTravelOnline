...
{
    public partial class XiRongList : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Convert.ToString(Session["Manager_UserId"]).Length == 0) Response.Redirect("/manage/Login.aspx", true);
            if (Convert.ToString(Session["Manager_UserRight"]).IndexOf("@6@6") == -1)
            {
        ...
        {
            string sqlstr = string.Format("SELECT XiRongOrder.xrorderid,XiRongOrder.flag,(select LineName from OL_Line where MisLineId=XiRongOrder.lineid) as XiRongLine,OL_Order.*,(select ISNULL(sum(PayPrice),0) from OL_PayMent where OrderId=OL_Order.OrderId) as pay from XiRongOrder,OL_Order where XiRongOrder.autoid=OL_Order.AutoId and OL_Order.OrderFlag<>'9' ", Convert.ToString(Session["Online_UserId"]));
            if (TB_Name.Text.Trim().Length > 0) sqlstr = string.Format("{0} and OL_Order.OrderName like '%{1}%' ", sqlstr, TB_Name.Text.Trim());
            if (TB_Line.Text.Trim().Length > 0) sqlstr = string.Format("{0} and OL_Order.LineName like '%{1}%' ", sqlstr, TB_Line.Text.Trim());
            if (TB_Bdate.Text.Length > 8)
            {
                try
                {
                    sqlstr = string.Format("{0} and OL_Order.OrderTime >= '{1}' ", sqlstr, Convert.ToDateTime(TB_Bdate.Text));
                }
                catch
                { }
            }
            ...
            if (TB_Edate.Text.Length > 8)
            {
                try
                {
                    sqlstr = string.Format("{0} and OL_Order.OrderTime <= '{1}' ", sqlstr, Convert.ToDateTime(TB_Edate.Text).AddDays(1));
                }
                catch
                { }
            }
