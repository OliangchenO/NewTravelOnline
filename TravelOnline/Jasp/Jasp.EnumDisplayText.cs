...
namespace Belinda.Jasp
{
    ...
    public class EnumDescription : Attribute
    {
        ...
        /// 获得指定枚举类型中，指定值的描述文本。
        /// </summary>
        /// <param name="enumValue">索引值</param>
        /// <param name="enumClass">枚举类型</param>
        /// <returns>描述字符串</returns>
        public static string GetFieldText(object enumValue, Type enumClass)
        {
            EnumDescription[] descriptions = GetFieldTexts(enumClass, SortType.Default);
            foreach (EnumDescription ed in descriptions)
            {
                if (ed.EnumValue == Jasp.DataConverter.CLng(enumValue))
                {
        ...
        /// </summary>
        /// <param name="enumValue">枚举值，不要作任何类型转换</param>
        /// <returns>在枚举中的索引值</returns>
        public static int GetFieldValue(object enumValue)
        {
            EnumDescription[] descriptions = GetFieldTexts(enumValue.GetType(), SortType.Default);
            foreach (EnumDescription ed in descriptions)
            {
                if (ed.fieldIno.Name == enumValue.ToString()) return ed.EnumValue;
            }
            return -1;
        }
        /// <summary>
        /// 获得指定枚举类型中，指定值的描述文本。
        /// </summary>
        /// <param name="EnumDisplayText">描述字符串</param>
        /// <param name="enumClass">枚举类型</param>
        /// <returns>在枚举中的索引值</returns>
        public static int GetFieldValue(string EnumDisplayText, Type enumClass)
        {
            EnumDescription[] descriptions = GetFieldTexts(enumClass, SortType.Default);
            foreach (EnumDescription ed in descriptions)
            {
                if (ed.EnumDisplayText == EnumDisplayText) return ed.EnumValue;
            }
            return -1;
        }
        /// <summary>
        ...
        /// <param name="enumClass">枚举类型</param>
        /// <returns>描述字符串</returns>
        public static string GetFieldTextTwo(int enumValue, Type enumClass)
        {
            EnumDescription[] descriptions = GetFieldTexts(enumClass, SortType.Default);
            foreach (EnumDescription ed in descriptions)
            {
