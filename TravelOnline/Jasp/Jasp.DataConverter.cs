...
namespace Belinda.Jasp
{
    ...
    public static class DataConverter
    {
        ...
        #region 将数据转换成十进制数字，十进制数是由符号、数值和比例因子组成的浮点值，数值的每一位的范围都是 0 到 9，比例因子指示分隔数值的整数和小数部分的浮点小数点的位置
        /// <summary>
        ...
        {
            return ((Convert.IsDBNull(input) || object.Equals(input, null)) ? 0 : CLng(input.ToString()));
        }
        /// <summary>
        /// 将数据转换成长整型数字
        /// </summary>
        ...
        {
            return ((Convert.IsDBNull(input) || object.Equals(input, null)) ? defaultValue : CLng(input.ToString(), defaultValue));
        }
        /// <summary>
        /// 将数据转换成长整型数字，如果数据无法转换，则返回输入的默认值
        /// </summary>
