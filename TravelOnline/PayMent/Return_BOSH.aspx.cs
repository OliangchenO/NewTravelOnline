...
{
    public partial class Return_BOSH : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            string orderNum = Request.QueryString["merOrderNum"];
            string merOrderNum = orderNum.Substring(10);//订单号
            string tranSerialNo = Request.QueryString["tranSerialNo"];              //流水号
            string merOrderAmt = Request.QueryString["merOrderAmt"];            //获取总金额
            string tranResult = Request.QueryString["tranResult"];
            ...
            {
                SaveErrorToLog("上海银行数据库更新错误", "out_trade_no=" + merOrderNum);
                return;
            }
            ...
            {
                SqlQueryText = string.Format("select PayPrice from OL_PayMent where PayType='BOSH' and OrderId='{0}' and TradeNo='{1}'", OrderID, tranSerialNo);
                ...
                if (DS.Tables[0].Rows.Count == 0)
                {
                    ...
                    Sql.Add(string.Format("insert into OL_PayMent (OrderId,TradeNo,Buyer,PayPrice,PayTime,PayContent,PayType) values ('{0}','{1}','{2}','{3}','{4}','{5}','BOSH')",
                        OrderID,
                        tranSerialNo,
                        "",
                        merOrderAmt,
                    ...
                    Sql.Add(string.Format("insert into OL_OrderLog (OrderId,LogTime,LogContent) values ('{0}','{1}','在线支付成功，付款金额：{2}，上海银行交易号：{3}')",
                        OrderID,
                        DateTime.Now.ToString(),
                        merOrderAmt,
                        tranSerialNo
                    ...
                    if (MyDataBaseComm.Transaction(SqlQuery) == true)
                    {
                        ...
                        SaveErrorToLog("上海银行支付成功", "trade_no=" + tranSerialNo);
                        string url = string.Format("/fifthstep.html?orderid={0}", OrderID);
                        Response.Redirect(url, true);
                    }
                    else
                    {
                        SaveErrorToLog("上海银行支付数据库更新错误", "trade_no=" + tranSerialNo);
                ...
                {
                    SaveErrorToLog("上海银行支付记录已存在", "trade_no=" + tranSerialNo);
                    string url = string.Format("/fifthstep.html?orderid={0}", OrderID);
                    Response.Redirect(url, true);
                }
            }
            ...
            {
                SaveErrorToLog("上海银行支付状态态错误！", "trade_status=" + tranResult);
                SaveErrorToLog("上海银行支付失败！", "trade_no=" + tranSerialNo);
