...
namespace TravelOnline.NewPage.coupon
{
    public partial class CaptureCoupon : System.Web.UI.Page
    {
        ...
        private void captureCoupon()
        {
            ...
            if (DS1.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    Response.Write("<script>alert('领取失败，你已经领用过此优惠券！');window.location.href='/ad/double_11/';</script>");
                    Response.End();
                }
                ...
                {
                    Response.Write("<script>alert('领取成功，可至首页-我的青旅-我的优惠券，查看所领取的优惠券！');window.location.href='/users/mycoupon.aspx';</script>");
                    Response.End();
                }
                else
                {
                    Response.Write("<script>alert('领取失败，请稍后再试！');window.location.href='/ad/double_11/';</script>");
                    Response.End();
                }
            }
            else
            {
                Response.Write("<script>alert('领取失败，优惠券不存在！');window.location.href='/ad/double_11/';</script>");
                Response.End();
            }
        }
    }
