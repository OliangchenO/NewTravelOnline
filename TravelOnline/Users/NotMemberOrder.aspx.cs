...
{
    public partial class NotMemberOrder : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            string SqlQueryText = string.Format("select * from OL_LoginUser where ThirdPartyType='NotMember' and ThirdPartyID = '{0}'" , MyDataBaseComm.StripSQLInjection(Request.QueryString["mobilePhone"].Trim()));
            ...
            else
            {
                ...
                LoginUser.RegistUser RUser = new LoginUser.RegistUser
                {
                    Id = id,
                    UserName = "新用户",
                    ThirdPartyID = MyDataBaseComm.StripSQLInjection(Request.QueryString["mobilePhone"].Trim()),
                ...
                else
                {
                    ...
                    Session["Online_ThirdPartyID"] = MyDataBaseComm.StripSQLInjection(Request.QueryString["mobilePhone"].Trim());
                ...
                if (DataBinder.Eval(e.Row.DataItem, "OrderFlag").ToString() == "1" || DataBinder.Eval(e.Row.DataItem, "OrderFlag").ToString() == "2")
                {
                    ...
                    else
                    {
                        if (Yue > 0)
                        {
                            ...
                            {
                                e.Row.Cells[8].Text = string.Format("<a class=pay1 href=\"/Pay/PayNow.aspx?OrderId={0}\" target=_blank>付款</a>", DataBinder.Eval(e.Row.DataItem, "OrderId"));
                            }
